...
export default function story(state, onChange) {
  ...
  const render = () => {
    const diff = null;
    const oldFiles = git.show().files;
    const newFiles = git.getAll();

    console.log(JSON.stringify(git.show().files, null, 2));
    console.log(JSON.stringify(git.getAll(), null, 2));
...
function renderCommits(git, editMode) {
  ...
  function process(commit, indent = 0) {
    let html = '';
    const { hash, message, derivatives } = commit;
    const cssClasses = ['commit-dot'];

    if (git.head() === hash) {
      cssClasses.push('commit-dot-head');
      if (editMode) {
        cssClasses.push('commit-dot-edit');
      }
    }
